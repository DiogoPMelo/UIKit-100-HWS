...
class ViewController: UIViewController {
    ...
    @objc func showPoints () {
        let ac = UIAlertController(title: nil, message: "Your current score is \(score) points", preferredStyle: .alert)
        ac.addAction(UIAlertAction(title: "Back to game", style: .default, handler: { action in }))
        present(ac, animated: true)
    }

    @objc func buttonTapped(_ sender: UIButton) {
        UIView.animate(withDuration: 0.5, delay: 0, usingSpringWithDamping: 5, initialSpringVelocity: 3, options: [], animations: {
            sender.transform = CGAffineTransform(scaleX: 0.9, y: 0.9)
        }) { finished in
            self.processTap(sender)
            sender.transform = .identity
        }
    }
    
    func processTap(_ sender: UIButton) {
        var title: String
        var message: String
    ...
    func askQuestionOrFinish (action: UIAlertAction? = nil) {
        ...
        if questionsAnswered.isMultiple(of: QUESTIONS_LIMIT) {
            let title = score <= 0 ? "GAME OVER" : "Congratulations!"
